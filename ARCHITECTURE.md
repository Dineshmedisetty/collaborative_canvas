...
Each room maintains:
- **Operations Array**: Complete history of all operations
- **Current Index**: Position in history for undo/redo
- **Users Map**: Active users in the room
- **Active Strokes Map**: Strokes currently being drawn

### Client State

Each client maintains:
- **Operation History**: Synced from server
- **Current Index**: Synced from server
- **Active Strokes**: Local tracking of remote active strokes
- **User Cursors**: Cursor positions of other users
- **Canvas State**: Rendered canvas (derived from operations)

### Synchronization

1. **On Join**: Server sends full canvas state to new client
2. **On Operation**: Server broadcasts updated state to all clients
3. **On Undo/Redo**: Server broadcasts full operations array and current index
4. **On Draw**: Server broadcasts stroke events in real-time (before adding to history)
...
- Add user permissions (read-only, draw, admin)
...
### Scaling Options
1. **Redis Adapter**: Share state across multiple server instances
2. **Database**: Persist operations to database
3. **Load Balancer**: Distribute WebSocket connections
4. **CDN**: Serve static files
5. **Message Queue**: Handle high-frequency events
